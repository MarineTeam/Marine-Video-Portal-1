...
export async function sendShareLinksEmail({ to, items, expiresInHours }) {
  ...
  return send({ to, ...shareLinksEmail({ items, expiresInHours }) });
}
...
function bundleEmail({ bundleUrl, items, expiresInHours }) {
  const hrs = Number(expiresInHours) || 72;
...
<html>
  <body style="margin:0;background:#070b14;font-family:-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;color:#e5e7eb;">
    <div style="max-width:480px;margin:0 auto;padding:32px 24px;">
      ...
      <p style="margin:0 0 20px;line-height:1.5;color:#cbd5e1;">
        Sign in once with <strong>this email address</strong> to see everything shared with you in one place.
      </p>
...
export async function sendBundleEmail({ to, bundleUrl, items, expiresInHours }) {
  ...
  return send({ to, ...bundleEmail({ bundleUrl, items, expiresInHours }) });
}
