...
// A THIRD limiter, for text other people will read (comments). The flood guard
// above would let one person post 60 comments in ten seconds — a wall of text
// under a sermon before a moderator could react. 30 an hour is generous for a
// conversation and tight for spam; the same figure the sibling repos use.
// Fails open like the others: the viewer gate in front is the access control.
const writingLimiter = new Ratelimit({
  redis,
...
export async function allowWriting(identifier) {
  try {
    const { success } = await writingLimiter.limit(identifier);
    return success;
  } catch (e) {
    return true;
  }
