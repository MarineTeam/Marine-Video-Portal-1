...
import { isAdmin, isLikelyEmail } from './auth';
import { scopeForEmail } from './staffScopeStore';
import { contentOfScope } from './staffScopeRules';
...
// groupId -> group record, without members: one read, for the scope checks.
export async function loadGroupsById() {
  const all = (await redis.hgetall(k(GROUPS_KEY))) || {};
  const out = {};
  for (const group of Object.values(all).map(parseRecord).filter(Boolean)) out[group.id] = group;
  return out;
}

// One group's members.
export async function groupMembers(groupId) {
  return (await redis.smembers(membersKey(String(groupId)))) || [];
}

// The group ids one person is in (the reverse index).
export async function groupIdsForEmail(email) {
  const e = normalizeEmail(email);
  if (!e) return [];
  return (await redis.smembers(userGroupsKey(e))) || [];
}
...
// What a group-scoped staff member gets when their scope cannot be read:
// nothing. Unlike the viewer path below, this one fails CLOSED — a limit that
// widened to the whole library on a Redis blip would not be a limit.
export const DENY = Object.freeze({ restricted: true, groupIds: [], collectionIds: [], videoIds: [] });

// Resolve what one viewer may see. `staff` short-circuits: owners and staff
// get UNRESTRICTED — unless the staff member is limited to certain groups
// (lib/staffScopeRules.js), who get what those groups grant.
//
...
export async function resolveAccess(email, { staff = false } = {}) {
  if (staff) {
    const e = normalizeEmail(email);
    if (!e || isAdmin(e)) return UNRESTRICTED;
    try {
      const scope = await scopeForEmail(e);
      if (!scope) return UNRESTRICTED;
      return contentOfScope(scope, await loadGroupsById());
    } catch {
      return DENY;
    }
  }
