...
// notice and extend it. Revoke remains a hard delete (removes the key and
// the active_shares membership immediately) — extend can therefore never
// resurrect a revoked link, only a merely-expired one.
...
// scans the active_shares set, same pattern the admin shares list already uses).
export async function listActiveSharesForEmail(email) {
  ...
  for (const id of ids) {
    ...
    if (data.email === email) out.push({ shareId: id, ...data });
  }
  return out;
}
