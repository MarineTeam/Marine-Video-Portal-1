...
// All non-revoked, non-expired shares currently addressed to this video,
// regardless of which flow created them (Create Link, Bulk Share, or the
// per-video Private list). A general video-scoped lookup, not list-specific
// — private-list.js further filters this to its own `privateList: true`
// tagged shares, since the list only ever tracks tokens it created itself.
export async function listActiveSharesForVideo(videoId) {
  const ids = await redis.smembers(k('active_shares'));
  const shares = await getShares(ids);
  return shares.filter((s) => s.videoId === videoId && !s.revoked && !isExpired(s));
}
...
// Admin-triggered garbage collection for bundles left behind by expiry.
// Revoke/delete already prune bundle membership as they happen (see
// pruneFromBundles above), but a bundle whose members simply EXPIRE with
// time is never touched — it just sits in Redis until its own grace-period
// TTL runs out, which keeps getting pushed further out if any sibling share
// was extended. There's no set enumerating bundle ids (nothing needs one on
// the hot path), so this scans `bundle:*` directly — fine for an occasional
// admin action, not something any request path does.
export async function sweepStaleBundles() {
  const prefix = k('bundle:');
  const bundleKeys = await redis.keys(`${prefix}*`);
  if (!bundleKeys.length) return { scanned: 0, removed: 0 };

  const bundles = await redis.mget(...bundleKeys);
  const allShareIds = new Set();
  bundles.forEach((b) => { if (b) b.itemIds.forEach((id) => allShareIds.add(id)); });
  const shareMap = new Map((await getShares([...allShareIds])).map((s) => [s.shareId, s]));
  ...
  bundleKeys.forEach((key, i) => {
    ...
    if (!bundle) return;
    const alive = bundle.itemIds.some((id) => {
      const s = shareMap.get(id);
      return s && !s.revoked && !isExpired(s);
    });
    if (!alive) {
      staleBundleIds.push(key.slice(prefix.length));
      staleEmails.push(bundle.email);
    }
  });

  if (!staleBundleIds.length) return { scanned: bundleKeys.length, removed: 0 };

  await redis.del(...staleBundleIds.map((id) => k(`bundle:${id}`)));

  // Only clear a pointer if it still points at the bundle we just removed —
  // guards against clobbering a newer bundle formed for the same email since.
  const pointerKeys = staleEmails.map((email) => k(`bundle_by_email:${email}`));
  const pointerValues = await redis.mget(...pointerKeys);
  const clearKeys = pointerKeys.filter((key, i) => pointerValues[i] === staleBundleIds[i]);
  if (clearKeys.length) await redis.del(...clearKeys);

  return { scanned: bundleKeys.length, removed: staleBundleIds.length };
}

// Reaps active_shares of any id whose share record has already fallen out of
// Redis past its grace-period TTL. getShares() already does this reap as a
// side effect of any normal read (the admin shares list, video-analytics,
// bundle sync), so this just forces that same sweep on demand.
export async function reapActiveShares() {
  const ids = await redis.smembers(k('active_shares'));
  if (!ids.length) return { scanned: 0, removed: 0 };
  const live = await getShares(ids);
  return { scanned: ids.length, removed: ids.length - live.length };
}
