import { listRoleGrants, ROLE_ADMIN, ROLE_MANAGER } from './roles';
...
// capability needed to approve the request. Admins and managers both qualify —
// notifying only admins would mean a manager who CAN action it never hears.
async function recipients() {
  const grants = await listRoleGrants();
  return grants
    .filter((g) => g.role === ROLE_ADMIN || g.role === ROLE_MANAGER)
    .map((g) => g.email)
    .filter(Boolean);
}
