...
export async function getAccess(email) {
  ...
  const capabilities = await resolveCapabilities(e);
  return { email: e, owner, capabilities, staff: capabilities.length > 0 };
}

export async function hasCapability(email, capability) {
  return (await resolveCapabilities(email)).includes(capability);
}

export async function isStaffUser(email) {
  return (await resolveCapabilities(email)).length > 0;
}
...
export async function requireCapability(req, res, capability) {
  ...
  return { session, email, owner: access.owner, capabilities: access.capabilities };
}
...
export async function holdersOf(cap, { rolesById, assignments, legacy } = {}) {
  const [roles, assigned, old] = await Promise.all([
    rolesById || loadRoles(),
  ...
  const legacyHolders = Object.entries(old)
    ...
    .map(([email]) => email);
  return emailsWithCapability({
    owners: [...ownerEmails(), ...legacyHolders],
