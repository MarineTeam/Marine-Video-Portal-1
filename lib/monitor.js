...
// Same, for outbound HTTP to a third party (currently bunny.net, via the
// instrumented fetch in lib/bunny.js). Redis commands are not the only cost of
// a request — on this app the Bunny round-trips usually dominate, and counting
// only Redis made the heaviest screens look like the cheapest.
...
// Full detail, for getServerSideProps props — serialized into the page, where
// there is no practical size ceiling.
function fullSnapshot(store, wallMs) {
  return { ...totals(store, wallMs), queries: store.queries, external: store.external };
}

// Counts only, for the response header. Response headers have platform size
// limits (and cost bytes on every single API call), so the per-command detail
// is deliberately omitted here — the panel renders aggregates for API calls.
...
// context so Redis/Bunny calls get attributed to this request, then attaches the
