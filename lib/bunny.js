...
// One page of the library, newest first, with bunny's count of the whole
// library. Anything that means "the library" reads every page through
// listAllVideos() in lib/videoLibrary.js — never page 1 alone, which is how
// every list in this portal used to stop at the newest 100 videos.
export async function listVideosPage({ page = 1, itemsPerPage = 100 } = {}) {
  ...
  const res = await fetch(
    `${BUNNY_API_BASE}/${process.env.BUNNY_LIBRARY_ID}/videos?${params}`,
    { headers: { AccessKey: process.env.BUNNY_API_KEY } }
  );
  if (!res.ok) throw new Error(`Bunny API error: ${res.status}`);
  const data = await res.json();
  return { items: Array.isArray(data?.items) ? data.items : [], totalItems: Number(data?.totalItems) || 0 };
}

// Whether a value is shaped like a bunny video id — the same check the
// functions below make before putting an id into a URL, for callers that want
// to answer "not found" for a malformed id instead of catching a throw.
...
// One video straight from Bunny, or null. One request whatever the size of
// the library — which is why every single-video page and route looks a video
// up here (via findVideo in lib/videoLibrary.js) rather than in a list.
