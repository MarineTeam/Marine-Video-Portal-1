...
export function signTusUpload(videoId, expiresInSeconds = 3600) {
  const libraryId = process.env.BUNNY_LIBRARY_ID;
  // Bunny's TUS auth expects the expiry as a millisecond timestamp (Date.now()-based).
  // Using seconds here makes Bunny treat the signature as already expired → HTTP 401.
  const expires = Date.now() + expiresInSeconds * 1000;
  const signature = crypto
    .createHash('sha256')
    .update(`${libraryId}${process.env.BUNNY_API_KEY}${expires}${videoId}`)
    .digest('hex');
