...
// Roles are custom, so the tests speak in capability sets. 'manager' and
// 'admin' are the two the old fixed tiers became; 'moderator' is a custom role
// holding comments:manage and nothing else.
vi.mock('../roles', async () => {
  const { ALL_CAPABILITIES, LEGACY_MANAGER_CAPABILITIES } = await vi.importActual('../capabilities');
  const caps = {
    viewer: [],
    moderator: ['comments:manage'],
    // A moderator who manages viewers but is limited to a group.
    scopedmod: ['comments:manage', 'viewers:manage'],
    manager: LEGACY_MANAGER_CAPABILITIES,
    admin: ALL_CAPABILITIES,
  };
  return {
    getAccess: async () => ({
      capabilities: [...caps[state.role]],
      staff: caps[state.role].length > 0,
      staffScope: state.role === 'scopedmod' ? ['g1'] : null,
    }),
  };
});
...
describe('reading', () => {
  ...
  it('keeps emails from a group-limited staff member, even one who manages viewers', async () => {
    as('smod@example.com', 'scopedmod');
    const res = await get();
    expect(res.body.comments[0].canDelete).toBe(true);
    expect(JSON.stringify(res.body)).not.toContain('jane@example.com');
  });

  it('keeps emails from a role that can moderate but not manage viewers', async () => {
    as('mod@example.com', 'moderator');
    const res = await get();
    expect(res.body.comments[0].canDelete).toBe(true);
    expect(JSON.stringify(res.body)).not.toContain('jane@example.com');
  });
...
describe('deleting', () => {
  ...
  it.each(['manager', 'admin', 'moderator'])('lets a %s remove anyone’s comment, and audits it', async (role) => {
