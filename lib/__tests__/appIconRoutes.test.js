...
vi.mock('../redis', () => ({
  ...
  redis: {
    hget: async (key, field) => {
      ...
      return key === 'pvp:app_icon' ? hash[field] ?? null : null;
    },
    hset: async (key, obj) => {
...
vi.mock('../roles', () => ({
  requireCapability: async (req, res, cap) => {
    ...
    // This repo's requireCapability returns { session, email, owner, capabilities }.
    return { session: {}, email: access.email, owner: false, capabilities: access.capabilities };
  },
}));
...
const CAP = { SETTINGS_MANAGE: 'settings:manage', VIDEOS_MANAGE: 'videos:manage' };
