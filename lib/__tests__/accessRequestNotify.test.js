...
beforeEach(() => {
  ...
  // Owners via ADMIN_EMAILS (vitest.config.js), a custom role that can
  // approve viewers, one that cannot, and an old Manager grant not yet
  // migrated to custom roles.
  hash('pvp:roles').set('people', JSON.stringify({ id: 'people', name: 'People', capabilities: ['viewers:manage'] }));
  hash('pvp:roles').set('media', JSON.stringify({ id: 'media', name: 'Media', capabilities: ['videos:manage'] }));
  hash('pvp:user_roles').set('people@example.com', JSON.stringify(['people']));
  hash('pvp:user_roles').set('media@example.com', JSON.stringify(['media']));
...
describe('access request notifications', () => {
  ...
  it('tells exactly the people who hold viewers:manage, since they can action it', async () => {
    await post('hello');
    const notified = state.emails.map((e) => e.to).sort();
