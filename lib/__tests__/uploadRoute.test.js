...
// requireCapability hands back the caller's whole capability list; a custom
// role may carry videos:manage without groups:manage.
vi.mock('../roles', () => ({
  requireCapability: async () => ({
    email: 'staff@example.com',
    owner: false,
    session: {},
    capabilities: state.canGrant ? ['groups:manage', 'videos:manage'] : ['videos:manage'],
  }),
}));
vi.mock('../groups', () => ({
  listGroupIds: async () => {
...
describe('upload with groups', () => {
  it('grants the new video to the chosen groups and audits it', async () => {
    ...
    expect(state.audit[0][0]).toBe('staff@example.com');
