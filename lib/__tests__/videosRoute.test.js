...
vi.mock('../videoLibrary', () => ({
  listAllVideos: async () => ({ videos: state.videos, truncated: state.libraryCut || false, total: state.videos.length }),
}));
...
vi.mock('../captionsStore', () => ({
  ...
  matchingTranslatedGuids: async (q) => {
    state.translatedQuery = q;
...
describe('searching translations', () => {
  it('finds a video by words said only in one of its translations', async () => {
    state.translated = ['c'];
    expect(ids(await search({ q: 'Oveja perdida' }))).toEqual(['c']);
    expect(state.translatedQuery).toBe('oveja perdida');
  });

  it('never surfaces a translated match the group filter removed', async () => {
    state.allowedGuids = ['a', 'b'];
    state.translated = ['c'];
    expect(ids(await search({ q: 'oveja' }))).toEqual([]);
  });
});
