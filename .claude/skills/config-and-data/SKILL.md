...
| `CRON_SECRET` | No | `lib/cronAuth.js` `cronSecret()` (trimmed), read per request by `pages/api/cron/transcripts.js` | Proves a scheduled-job call came from Vercel's cron runner, which sends it as `Authorization: Bearer …` | Inert — every `/api/cron/*` route answers 404, also when shorter than 16 characters (logged); transcripts are still collected when an admin opens the Videos tab | **Secret.** Request-time only, so no CI or vitest mirror is needed. 16+ random characters (`openssl rand -hex 24`); the schedule lives in `vercel.json` |
| `EMAIL_VERIFIED_BYPASS_EMAILS` | No | `lib/verification.js` `bypassEmails()` | Emails that always skip the optional email-verification check regardless of the toggle — a standing safety net mirroring `ADMIN_GEO_BYPASS_EMAILS` | Unset → empty list; nobody is bypassed (the check itself is still off by default) | Comma-separated, trimmed + lowercased. Staff are exempt unconditionally and need not be listed. Env changes need a redeploy, so arm this BEFORE enabling enforcement |
| `PODCAST_MEDIA_FILE` | No | `lib/bunny.js` `getVideoFileUrl`/`podcastMediaFile` | Which Bunny rendition the podcast feed links as its enclosure, e.g. `play_480p.mp4` or an audio rendition if the library exposes one | Defaults to `play_480p.mp4` | **Unverified against the live library** — nothing in this app referenced direct media files before, so which renditions exist is unknown. See README's podcast section for the two Bunny-side conditions that must hold |
...
> **Known gap (noted 2026-08-31):** this table was derived on 2026-07-10 and has not been backfilled for env vars added since. Missing rows include `GEO_WHITELIST`, `ADMIN_GEO_WHITELIST`, `ADMIN_GEO_BYPASS_EMAILS`, `RESEND_API_KEY`, `MAIL_FROM`, and `QUERY_MONITOR_ENABLED` — all of them are documented in README.md's env section and FEATURES.md's "Configuration knobs". Re-derive per "Provenance & maintenance" before treating this table as exhaustive.
...
| `pvp:roles` | hash: roleId → JSON `{ id, name, capabilities[], createdAt, updatedAt }` (added v1.28.0) | `lib/roles.js` `saveRole` / `deleteRole`, via `pages/api/admin/roles.js` (`roles:manage`); `lib/roleMigration.js` HSETNX of `admin-legacy` / `manager-legacy` | `lib/roles.js` `loadRoles` (on every gated request, for non-owners) | none | At most 50 roles (`MAX_ROLES`). Capabilities are normalised against `lib/capabilities.js` on write AND read, so a hand-edited record can't widen the catalog; a record that doesn't parse is ignored |
| `pvp:user_roles` | hash: lowercase email → JSON array of roleIds (added v1.28.0) | `lib/roles.js` `setRolesForEmail` (PATCH on `/api/admin/roles`), `deleteRole` (strips the id from everyone), `lib/roleMigration.js` | `lib/roles.js` `rolesForEmail` (on every gated request, for non-owners), `loadRoleAssignments` | none | At most 10 roles per person; ids with no live role are dropped on write. **`ADMIN_EMAILS` owners never appear here** — they hold everything without Redis being read, and the route refuses to assign them |
| `pvp:user_scope` | hash: lowercase email → JSON array of group ids (added v1.29.0) | `lib/staffScopeStore.js` `setScopeForEmail` (PATCH on `/api/admin/roles`; cleared with the last role) | `lib/roles.js` `getAccess` (non-owners holding a capability), `lib/groups.js` `resolveAccess(…, { staff: true })`, `loadStaffScopes` (Roles GET, `holdersOf`) | none | Absent = unlimited. An **empty list is stored, never deleted** — absent would mean the whole portal. Unparseable reads as `[]`; a read error gives no capabilities (fail closed). Owners are never read |
| `pvp:role_admins` | set of lowercase emails (v1.19.0–v1.27.0; **legacy**) | nothing since v1.28.0 — `lib/roleMigration.js` SREMs each address once it has been converted | `lib/roleMigration.js` `legacyCapabilitiesFor` (read-time fallback when someone has no custom role), `findLegacyAssignments` | none | Empties itself the first time someone opens Admin → Access → Roles; its members become holders of the editable "Admin" role (`admin-legacy`, every capability) |
| `pvp:role_managers` | set of lowercase emails (v1.19.0–v1.27.0; **legacy**) | as `role_admins` | as `role_admins` | none | Members become holders of the "Manager" role (`manager-legacy`, everything but `settings:manage` and `roles:manage`) |
...
| `pvp:access_requests` | hash email → JSON `{email, note, status, requestedAt, decidedAt, decidedBy}` (added 2026-08-31) | `lib/accessRequests.js` `submitRequest` (HSET, via `pages/api/access-request.js` — the ONLY viewer-writable admin-adjacent key), `decideRequest` (HSET), `deleteRequest` (HDEL) | `listRequests`/`getRequest` (HGETALL/HGET) via `pages/api/admin/access-requests.js` | none | One row per requesting email — re-asking updates in place. Pending queue capped at 500 (`MAX_PENDING`); note clamped to 500 chars and stripped of control characters |
| `pvp:public_videos` | set of video guids (added 2026-09-13) | `lib/publicVideos.js` `setPublicVideo`/`clearPublicVideo`, via `pages/api/admin/public-videos.js` (admin-only, `settings:manage`) | `isPublicVideo` in `lib/publicWatch.js`, `listPublicVideos` in `pages/api/admin/videos.js` GET | none | Bounded by admin action. **Reads fail CLOSED** — unique in this table; every other optional read fails open, but this one guards login-less access and a blip must never publish the library |
| `pvp:feed_token_email` | hash token → email (added 2026-09-13) | `lib/feedTokens.js` `getOrCreateToken`/`rotateToken` (HSET), `revokeToken` (HDEL), via `pages/api/feed-url.js` and viewer removal in `pages/api/admin/viewers.js` | `resolveToken` on every `pages/api/feed/[token].js` fetch | none | One row per subscribing viewer. Token is a 256-bit bearer credential; resolution **fails closed** |
| `pvp:feed_token_by_email` | hash email → token — reverse index of the above (added 2026-09-13) | maintained alongside it by the same `lib/feedTokens.js` functions | `getExistingToken`, so a viewer keeps a stable feed URL across visits | none | One row per subscribing viewer; both rows are deleted when a viewer is removed |
| `pvp:video_meta` | hash videoId → JSON `{notes, chapters:[{seconds,label}]}` (added 2026-09-13) | `lib/videoMetaStore.js` `setVideoMeta` (HSET, or HDEL when both fields are cleared), `clearVideoMeta` (HDEL on video delete), via `pages/api/admin/videos.js` PUT/DELETE | `listVideoMeta` in `pages/api/admin/videos.js` GET and `pages/api/videos.js` (search only), `getVideoMeta` in `pages/watch/video/[id].js` | none | One row per video that HAS chapters or notes — an entry with neither is deleted, so no dead rows. Chapters capped at 100/video, labels 120 chars, notes 2000 chars. Reads fail soft to `{}` / `null` |
| `pvp:video_schedule` | hash videoId → JSON `{publishAt, expiresAt}` ms epoch, either nullable (added 2026-08-31) | `lib/schedule.js` `setSchedule` (HSET, or HDEL when both bounds are cleared), via `pages/api/admin/videos.js` PUT | `listSchedules` in `pages/api/videos.js` + `pages/api/admin/videos.js`, `getSchedule` in `pages/watch/video/[id].js` | none | One row per *scheduled* video only — an unscheduled video has no entry and is unaffected. Not pruned on video delete (a stale entry is inert; the videoId no longer resolves) |
| `pvp:require_email_verified` | `'1'` / `'0'` (added 2026-08-31) | `lib/verification.js` `setEnforcementEnabled`, via `pages/api/admin/verification.js` (admin-only, requires explicit confirm to enable) | `isEnforcementEnabled` on every library-surface request | none | Single value; **absent → OFF**. Read failure also yields OFF (fails open) — never let this fail closed |
| `pvp:email_verified_seen` | hash email → JSON `{verified: true\|false\|null, at}` (added 2026-08-31) | `lib/verification.js` `recordObservation` (HSET on each `/api/videos` hit; best-effort, swallowed on error) | `listObservations` → `summarizeObservations` for the Settings blast-radius panel | none | **Unbounded-growth candidate**, same profile as `viewer_last_seen`: one field per account that ever loaded the library, no pruning on viewer removal |
...
| `pvp:site_name` | string, cleaned + clamped to 40 chars (added 2026-09-03) | `lib/brandingStore.js` `setSiteName` (SET, or DEL when cleared back to the default), via `pages/api/theme.js` POST (admin-only) | `getSiteName()` — `pages/api/theme.js` GET (public), `pages/api/manifest.js`, `lib/mail.js`, `lib/push.js`, `pages/api/admin/broadcast.js` | none | Single value; absent → `DEFAULT_SITE_NAME` in `lib/branding.js`. Read failure also yields the default (fails soft) — the name decorates a header, an email and a push title, none worth a 500 |
...
| `pvp:comments:{videoId}` | hash comment id (`c` + base-36 time + random) → JSON `{id, email, name, text, at}` | `lib/commentsStore.js` `addComment` (via `POST /api/comments`) — a Lua script that refuses once the hash holds 500 | `listComments` / `getComment` via `GET` and `DELETE /api/comments`; the email decides "mine" and is shown only to `viewers:manage` holders | per-comment HDEL; the whole key on video delete (`pages/api/admin/videos.js`) | **Capped** at 500 per video; NOT removed with a viewer |
| `pvp:rl-writing:*` | @upstash/ratelimit sliding-window internals for text other people read | `lib/ratelimit.js` `allowWriting` — `slidingWindow(30, '1 h')` | same | sliding window | Bounded by active writers |
| `pvp:transcript_text_alt` | hash `{videoId}:{lang}` → that translation's words, ALREADY normalised (`normalizeSpokenText`) | `lib/captionsStore.js` `setTranscriptLanguage`, beside the cue row | `matchingTranslatedGuids` only — a Lua script searches it INSIDE Redis and returns just the matching ids, joined into `/api/videos`' spoken matches over the already-filtered list. Pre-normalised because Lua's `lower()` is not Unicode-aware | cleared by `clearTranscript` (which the video delete now calls) | One field per stored translation; never loaded into the app, so more languages do not grow what a search reads |
| `pvp:transcribe_pending` | hash videoId → epoch ms when transcription was queued | `lib/captionsStore.js` `markTranscribePending` (via `pages/api/admin/transcribe.js`) | `getTranscribePending` → `lib/transcribeQueue.js` `planCollection`, from the admin video list and `pages/api/cron/transcripts.js` | cleared on ingest, or after `PENDING_MAX_AGE_MS` (3 days) | Bounded by transcriptions queued; self-clearing |
| `pvp:transcribe_collecting` | string, a random `lock-…` token | `lib/captionsStore.js` `acquireCollectLock` (SET NX EX 300) | the release script only (compare-and-delete) | **EX 300** | One key at most; the collection lock (Decision 18 in `architecture-contract`) |
