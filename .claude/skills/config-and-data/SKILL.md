...
| `pvp:rl:*` | @upstash/ratelimit sliding-window internals | `lib/ratelimit.js:6-11` — `Ratelimit.slidingWindow(60, '10 s')`, `prefix: k('rl')` | same library | windows expire automatically | Self-expiring. Identifier = `{bucket}:{email-or-IP}`; buckets in use: `videos` (`pages/api/videos.js:12`), `share` (`pages/api/admin/share.js:14`), `upload` (`pages/api/admin/upload.js:13`). Limiter **fails open** on backend errors (`lib/ratelimit.js:26-33`) |
