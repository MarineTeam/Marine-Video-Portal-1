...
| `pvp:share:{uuid}` | JSON `{videoId, title, email, expiresAt}` + `viewedAt` after first view | `pages/api/admin/share.js:25-29` (SET with `ex:` TTL); `pages/api/admin/private-list.js` (SET, same shape — a Private-list member is just a share record); `pages/watch/[shareId].js:36` rewrites with the **remaining** TTL to stamp `viewedAt`; DEL on revoke `shares.js:32` | `pages/watch/[shareId].js:19`, `pages/api/admin/shares.js:16`, `pages/api/admin/private-list.js` (GET) | EX ≤ 30 days (`Math.min(expiresInHours, 720) * 3600`, `share.js:22`; private-list.js uses the same 720h cap unconditionally) | Self-expiring — safe |
