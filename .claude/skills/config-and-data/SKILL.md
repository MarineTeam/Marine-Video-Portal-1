...
| `pvp:share:{uuid}` | JSON `{videoId, title, email, expiresAt}` + `viewedAt` after first view | `pages/api/admin/share.js:25-29` (SET with `ex:` TTL); `pages/watch/[shareId].js:36` rewrites with the **remaining** TTL to stamp `viewedAt`; DEL on revoke `shares.js:32` | `pages/watch/[shareId].js:19`, `pages/api/admin/shares.js:16` | EX ≤ 30 days (`Math.min(expiresInHours, 720) * 3600`, `share.js:22`) | Self-expiring — safe |
| `pvp:active_shares` | set of share uuids | `share.js:30` (SADD); SREM on revoke `shares.js:33` | `shares.js:12` (SMEMBERS) | none — **lazily pruned**: `shares.js:17-20` SREMs any id whose `pvp:share:{id}` has expired, but only when an admin opens the shares list | Bounded in practice by the lazy prune; stale ids linger until the next admin GET |
| `pvp:audit_log` | list of `{at, actor, action, detail}` | `lib/audit.js:9-10` `logAudit` — LPUSH + LTRIM to cap 200. Call sites: `viewers.js` (viewer.add/remove), `share.js` (share.create), `shares.js` (share.revoke), `settings.js` (settings.homepage_count), `theme.js` (theme.update), `admin/videos.js` (video.rename/delete/collection), `admin/collections.js` (collection.create/delete) | `lib/audit.js:17` `getAudit` (via `pages/api/admin/audit.js:10`) | LTRIM cap 200 entries | Hard-capped — safe. `logAudit` never throws (`lib/audit.js:6-13`) |
...
| `pvp:rl:*` | @upstash/ratelimit sliding-window internals | `lib/ratelimit.js:6-11` — `Ratelimit.slidingWindow(60, '10 s')`, `prefix: k('rl')` | same library | windows expire automatically | Self-expiring. Identifier = `{bucket}:{email-or-IP}`; buckets in use: `videos` (`pages/api/videos.js:12`), `share` (`pages/api/admin/share.js:14`), `upload` (`pages/api/admin/upload.js:13`), `push` (`pages/api/push/subscribe.js`). Limiter **fails open** on backend errors (`lib/ratelimit.js:26-33`) |
