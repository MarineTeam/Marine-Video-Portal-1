...
5. **NEVER put an access decision in `proxy.js`, and never add an `app/` directory.** (Amended 2026-09-24: the owner accepted a sign-in-only `proxy.js` to reach Next 16 — see the postcss entry below and architecture-contract Decision 1. The original fence follows.) **NEVER add a `middleware.js` to "mitigate" the deferred Next.js alerts.** The 14 deferred Dependabot alerts are deferred *because* the vulnerable features (middleware among them) are absent. Adding middleware expands the attack surface and invalidates the entire deferral analysis in one move.
...
```bash
ls middleware.js                                  # expect: No such file or directory (proxy.js exists since 2026-09-24 — sign-in only; see fence 5)
