...
**Re-verified 2026-09-17** during a security audit, on `next@14.2.35`. All seven absence checks still hold: no `middleware.js`, no `app/` directory, no `i18n` in `next.config.js`, no `rewrites()`, no `next/image` import, no `next/script` import, no WebSocket handling. `npm audit` on a freshly resolved tree now reports 9 findings (1 critical, 6 high, 2 moderate) — the critical is `GHSA-2xp9-vwfh-vxw4`, unauthenticated RCE in the Image Optimization API via AVIF, which needs `next/image` and so is not reachable here; the Windows RCE `GHSA-p293-qw3h-jr36` needs a Windows host and Vercel is Linux. The remaining highs are `postcss` (arbitrary `.map` file read via `sourceMappingURL`) and `rollup` path traversal reached through `@sentry/nextjs` 7.x — both build-time, neither reachable from a request. **The deferral therefore still stands, but note what changed: `npm audit fix --force` now proposes `next@16.3.5` and `@sentry/nextjs@10.75.0`, which are exactly the versions the two sibling repos (fable-video, fable-video2) already run in production on this same Pages-Router codebase.** The migration path is no longer theoretical, so the cost side of this deferral has dropped; that is a decision for `security-currency-campaign`, not for a passing change.

**Caveat on the one thing that did change** | `next.config.js` gained a `headers()` block in the same audit (clickjacking fix). `headers()` is not `rewrites()` and appears in none of the deferred advisories, so it does not expire the rationale — but the "no `rewrites()`" line above is now the only config-level check left, and the next person to add one must re-open this entry.
