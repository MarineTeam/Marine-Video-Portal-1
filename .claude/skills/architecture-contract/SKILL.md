...
- [ ] Deleting a collection clears it from every group scope that granted it (`pruneCollectionFromGroups`), and never throws into the delete path. A grant naming a collection that no longer exists is clutter at best and a grant a reused id inherits at worst — the same no-orphans rule the per-viewer keys follow.
...
- [ ] `pvp:rating_counts` holds integers and no email, `/api/rating` returns no totals to a viewer, and the counter update sits outside the error path that answers them (Decision 16).
