...
- **Per-video transcripts** live in **two** keys, `pvp:transcripts` (cues with timings, read one field at a time by `/api/transcript/[id]`) and `pvp:transcript_text` (the same words as one string, read by search), split across `lib/captions.js` (pure) and `lib/captionsStore.js` (Redis). Two keys rather than one because the access patterns differ: search needs one `hgetall` and **none of the timings**, and cue arrays run ~1,500 per 90-minute service — unlike chapters and notes, which are small and always read together, hence their single hash. Transcript matching joins title and note matching in `/api/videos` **after** the group and schedule filters, so it inherits the same guarantee. **Passage matching (2026-09-23)** is a fourth OR in the same filter: a query that IS a scripture reference (`parsePassageQuery`, `lib/scripture.js`, parsed once per search) also matches a title or notes citing an overlapping passage — over the same already-filtered list, so it inherits the guarantee too, and it only adds matches. `lib/scripture.js` is pure and client-bundled (the watch page lists passages), so it must not use regex lookbehind: a parse-time SyntaxError in Safari before 16.4. Verify: `npm test -- scripture videosRoute searchLink`; `grep -n "(?<" lib/scripture.js` prints nothing. **Word stems** are a fifth OR in the same filter (`lib/stem.js`, titles and notes, additive), skipped for a passage query — stems would widen "Philippians 2" to the whole book (`npm test -- stem videosRoute`). **Browse by book** is `GET /api/videos?index=books` — a MODE of that route, not a route of its own, so it passes through the one copy of the gate (approval, region, verified email) and is counted over `ordered`, after the group and schedule filters. A count is information ("Philippians (3)" says three videos exist); counting anything wider would leak what those filters hide. **Other languages** (2026-09-23) live in two MORE keys: `pvp:transcripts_alt` (`{guid}:{lang}` → cue array, every non-default track bunny produced) and `pvp:transcript_langs` (guid → `{default, all[]}`). The default track stays in `pvp:transcripts` and is the only one `pvp:transcript_text` indexes. `transcripts_alt` is a separate hash, not extra fields in `transcripts`, because the sibling repos read `transcripts` field names with `hkeys` and a `guid:lang` field there reads as a video that does not exist — this repo has no such read today, and keeping the layouts the same stops one being added later from quietly breaking. `clearTranscript` reads `transcript_langs` BEFORE deleting, since it is the only record of which `transcripts_alt` fields exist. Written only by `/api/admin/transcribe`, which spends money ($0.10/min of video at bunny) and therefore uses `allowCostly` rather than the shared 60-per-10s `allow`.
...
- The parser **reports the lines it could not read** rather than dropping them silently. A chapter list that is quietly short is a support call. The suggestion reader follows the same rule, and for the same reason it reads bunny's field names liberally: `title`/`start` come from bunny's docs and this path has never run against a live job, so a docs/reality mismatch must surface as "3 suggestions could not be read", never as an empty list with no explanation.
- **The AI proposes; a person accepts.** Two writers for one field is how hand-written work gets silently replaced — the admin who typed `24:15 Sermon` would have no way to notice a transcription job had overwritten it, and no way to get it back. Keeping acceptance in the admin's hands costs one click and removes the whole class of failure. Anything future that generates content overlapping what a person authors here goes somewhere the person's copy is not.
...
- Let the transcription path write chapters directly → a hand-typed list is replaced by a job the admin ran for the captions, silently and unrecoverably.
...
- [ ] Deleting a collection clears it from every group scope that granted it (`pruneCollectionFromGroups`), and never throws into the delete path.
...
- [ ] A request for a transcript language the video does not have is answered with the default track AND `missing: true`, never silently; the collector stores nothing until the default track parses, and one unreadable translation costs only itself (`lib/__tests__/transcriptRoute.test.js`, `lib/__tests__/transcriptCollect.test.js`).
...
- [ ] `POST /api/progress` accepts only a bunny video id, a finite position and a bounded title, and writes through `saveProgress` (`lib/progressStore.js`), which keeps each `pvp:progress:{email}` hash at `MAX_PROGRESS_ENTRIES` (1,000). Deleting a video clears its schedule (`clearSchedule`) and watermark override (`setVideoWatermarkMode(id, 'default')`) with its other per-video rows (`npm test -- progress progressRoute progressStore.redis videoDeleteRoute`).
...
| 2 | `/api/progress` writes to Redis on every playback tick (~8 s per active viewer) (`pages/api/progress.js`) | **Closed 2026-09-24.** Rate-limited by the shared `allow(callerId(...))` flood guard (this row was stale: the guard was already there), and now bounded too: only a bunny video id (`isVideoId`), a title of at most 200 characters, and at most 1,000 videos per viewer (`lib/progressStore.js` — one Lua script on the common path, least-recently-watched eviction at the cap) | — |
| 3 | `pvp:viewer_last_seen` and per-user `pvp:progress:{email}` hashes grow unbounded — no TTL, no pruning, entries persist after viewer removal | Partly mitigated: `sweepOrphanedViewerData` in `lib/maintenance.js` clears orphaned per-viewer hashes for EVERY family listed in its `VIEWER_KEY_PREFIXES` (progress, mylist and ratings today — `lib/__tests__/ratings.test.js` asserts all three are listed) — adding a per-viewer key without adding it there is how this weak point was created, and viewer removal now also clears group memberships (`removeUserFromAllGroups`). `viewer_last_seen` is still unbounded | TTLs on `viewer_last_seen` |
