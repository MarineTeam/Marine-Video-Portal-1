...
- **Email delivery** _(opt-in)_ — a checkbox on the share form emails the link straight to the recipient via Resend; each active link also has a **Resend email** button to re-deliver it. Best-effort (a mail failure never blocks link creation) and inert unless `RESEND_API_KEY` is set.
...
- **Smoke tests** — Vitest coverage for the auth check, video-ordering logic, and theme helpers.
