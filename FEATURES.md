...
- **AI chapters are not accepted automatically** — bunny can generate chapters from the transcript and the transcribe call deliberately turns that off. Chapters here are admin-authored (`lib/videoMeta.js`); a second writer for the same field is how hand-written ones get silently replaced.
